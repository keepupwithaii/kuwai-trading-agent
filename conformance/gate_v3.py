#!/usr/bin/env python3
"""
The mandatory binary build-conformance gate for commitment-v3.

Reuses all 15 v2 gates re-targeted at commitment-v3/ (paths swapped only,
...
# __pycache__ into commitment-v3/ (C-MANIFEST demands an exact member set).
...
sys.path.insert(0, str(REPO / "commitment-v3" / "code"))
...
def _c_manifest():
    ...
    if (SEALED / "README.md").exists() or (SEALED / ".gitignore").exists():
        return False, "README/.gitignore must not be inside commitment-v3/"
...
@check("C-SECRETS")
def _c_secrets():
    out = REPO / "conformance" / "secret-scan-report-v3.txt"
...
def _plumbing():
    ...
    logp = REPO / "conformance" / "_plumbing_run_v3.log"
    if logp.exists():
        logp.unlink()
    br = StubBroker()
    ...
    for art in ("commitment-v3.zip", "commitment-v3.zip.ots"):
        if (REPO / art).exists():
...
def _c_perception_wired():
    ...
    logp = REPO / "conformance" / "_wired_run_v3.log"
...
def _c_loop_failsafe_transient():
    ...
    logp_a = REPO / "conformance" / "_failsafe_transient_run_v3.log"
    if logp_a.exists():
        logp_a.unlink()
    ...
    logp_b = REPO / "conformance" / "_failsafe_auth_run_v3.log"
    if logp_b.exists():
        logp_b.unlink()
    auth_calls = {"n": 0}
...
def main() -> int:
    print("=== KUWAI commitment-v3 BINARY CONFORMANCE GATE (17 gates) ===")
    all_pass = True
