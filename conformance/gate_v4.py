#!/usr/bin/env python3
"""
The mandatory binary build-conformance gate for commitment-v4.

Reuses all 15 v2 gates re-targeted at commitment-v4/ (paths swapped only,
...
# __pycache__ into commitment-v4/ (C-MANIFEST demands an exact member set).
...
SEALED = REPO / "commitment-v4"
sys.path.insert(0, str(REPO / "commitment-v4" / "code"))
...
def _c_manifest():
    ...
    if (SEALED / "README.md").exists() or (SEALED / ".gitignore").exists():
        return False, "README/.gitignore must not be inside commitment-v4/"
...
@check("C-SECRETS")
def _c_secrets():
    out = REPO / "conformance" / "secret-scan-report-v4.txt"
...
def _plumbing():
    ...
    logp = REPO / "conformance" / "_plumbing_run_v4.log"
    if logp.exists():
        logp.unlink()
    br = StubBroker()
    ...
    for art in ("commitment-v4.zip", "commitment-v4.zip.ots"):
        if (REPO / art).exists():
...
def _c_perception_wired():
    ...
    logp = REPO / "conformance" / "_wired_run_v4.log"
...
def _c_loop_failsafe_transient():
    ...
    logp_a = REPO / "conformance" / "_failsafe_transient_run_v4.log"
    if logp_a.exists():
        logp_a.unlink()
    ...
    logp_b = REPO / "conformance" / "_failsafe_auth_run_v4.log"
    if logp_b.exists():
        logp_b.unlink()
    auth_calls = {"n": 0}
...
def main() -> int:
    print("=== KUWAI commitment-v4 BINARY CONFORMANCE GATE (17 gates) ===")
    all_pass = True
