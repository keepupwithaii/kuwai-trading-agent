#!/usr/bin/env python3
"""
The mandatory binary build-conformance gate for commitment-v6.

Reuses all 15 v2 gates re-targeted at commitment-v6/ (paths swapped only,
...
# __pycache__ into commitment-v6/ (C-MANIFEST demands an exact member set).
...
SEALED = REPO / "commitment-v6"
sys.path.insert(0, str(REPO / "commitment-v6" / "code"))
...
def _c_manifest():
    ...
    if (SEALED / "README.md").exists() or (SEALED / ".gitignore").exists():
        return False, "README/.gitignore must not be inside commitment-v6/"
...
@check("C-SECRETS")
def _c_secrets():
    out = REPO / "conformance" / "secret-scan-report-v6.txt"
...
def _c_halt_safe():
    ...
    # v6 (2026-05-29): the v5 sub-test asserted a-fault -> hard HALT
    # 0 retries. v6 deliberately changes a_grounded specifically: it
    # now retries up to 2x on a spurious fault then resolves to pass.
    # Other bright-lines (b/c/d/e/f) are unchanged. Test the bright-
    # line-hard-HALT property using d_no_must_never_breach instead.
    good = {"dimensions": {d: {"verdict": "pass", "why": "ok"} for d in DIMS}}
    good["dimensions"]["d_no_must_never_breach"] = {
        "verdict": "fault",
        "why": "ADDRESSES_AUDIENCE: response addresses readers"}
    r3 = council_mod.run_council({}, {}, {},
                                 lambda attempt: json.dumps(good))
    if r3.disposition != "halt" or r3.retries_consumed != 0:
        return False, "bright-line d-fault not hard-HALT/0-retry"
    return True, ("fail-safe-to-HALT; retry trigger scope correct; "
                  "bright-line b/c/d/e/f still 0-retry "
                  "(a_grounded retry tested separately)")
...
def _plumbing():
    ...
    logp = REPO / "conformance" / "_plumbing_run_v6.log"
    if logp.exists():
        logp.unlink()
    br = StubBroker()
    ...
    for art in ("commitment-v6.zip", "commitment-v6.zip.ots"):
        if (REPO / art).exists():
...
def _c_perception_wired():
    ...
    logp = REPO / "conformance" / "_wired_run_v6.log"
...
def _c_loop_failsafe_transient():
    ...
    logp_a = REPO / "conformance" / "_failsafe_transient_run_v6.log"
    if logp_a.exists():
        logp_a.unlink()
    ...
    logp_b = REPO / "conformance" / "_failsafe_auth_run_v6.log"
    if logp_b.exists():
        logp_b.unlink()
    auth_calls = {"n": 0}
...
# ---- C-A-GROUNDED-RETRY (v6 NEW) ----------------------------------------
# When the deterministic linter returns grounded=true and the synth emits a
# generic a_grounded:fault:UNGROUNDED with no linter-flagged cite named,
# the council MUST retry rather than terminally halt on the first draw.
# Validates the core v6 fix: non-deterministic bright-lines cannot
# zero-retry-terminate the chain.
@check("C-A-GROUNDED-RETRY")
def _c_a_grounded_retry():
    sys.path.insert(0, str(SEALED / "code"))
    import importlib, council as _council
    importlib.reload(_council)
    DIMS = _council.DIMS

    linter_grounded_true = {
        "grounded": True,
        "citation_map": {
            "A_market_bars AMZN 2026-05-22 close 266.27": True,
            "C_news_catalyst id 52765375 Memorial Day closure": True,
            "O_own_account 2026-05-25T13:35": True,
        },
        "ungrounded_reason": "",
    }
    # Stub synth: emit spurious a_grounded:fault on first attempt, then
    # clean pass on retries (proves the retry actually FIRED and the
    # subsequent draw resolved cleanly).
    state = {"calls": 0}
    def stub(attempt):
        state["calls"] += 1
        rep = {"schema": "council-v1",
               "dimensions": {d: {"verdict": "pass", "why": "ok"}
                              for d in DIMS}}
        if attempt == 1:
            rep["dimensions"]["a_grounded"] = {
                "verdict": "fault",
                "why": "UNGROUNDED (no specific cite named)",
            }
        return json.dumps(rep)
    r = _council.run_council({}, {}, linter_grounded_true, stub)
    if r.disposition != "proceed":
        return False, (f"council halted on a spurious a_grounded fault "
                       f"instead of retrying: disposition={r.disposition!r}, "
                       f"halt_reason={r.halt_reason!r}, "
                       f"retries={r.retries_consumed}")
    if r.retries_consumed < 1:
        return False, (f"council did not consume any retry "
                       f"(retries_consumed={r.retries_consumed})")
    if state["calls"] < 2:
        return False, (f"synthesiser_callable was only invoked "
                       f"{state['calls']} times (expected >=2)")
    return True, (f"spurious a_grounded:fault retried; subsequent draw "
                  f"resolved to proceed; retries_consumed="
                  f"{r.retries_consumed}, synth calls={state['calls']}")


# ---- C-A-GROUNDED-SPURIOUS-OVERRIDE (v6 NEW) ----------------------------
# When the synth emits a spurious a_grounded:fault on ALL retries (the bug
# is deterministic in this fixture, not just probabilistic) and the linter
# remains grounded=true, the council MUST resolve a_grounded to pass and
# proceed -- the v6 spurious-fault reconciliation. The chain must NOT
# terminate on persistent stochastic noise when the deterministic linter
# disagrees.
@check("C-A-GROUNDED-SPURIOUS-OVERRIDE")
def _c_a_grounded_spurious_override():
    sys.path.insert(0, str(SEALED / "code"))
    import importlib, council as _council
    importlib.reload(_council)
    DIMS = _council.DIMS
    ...
    linter_grounded_true = {
        "grounded": True,
        "citation_map": {
            "A_market_bars GOOGL 2026-05-20 close 388.86": True,
            "A_market_bars AAPL 2026-05-22 close 308.81": True,
            "C_news_catalyst id 52765748 Tom Lee Mag-7 bear over": True,
            "C_news_catalyst id 52766095 El-Erian policy put dead": True,
            "O_own_account 2026-05-25T14:35": True,
        },
        "ungrounded_reason": "",
    }
    # Stub synth: spurious a_grounded:fault on EVERY attempt.
    state = {"calls": 0}
    def stub(attempt):
        state["calls"] += 1
        rep = {"schema": "council-v1",
        ...
        rep["dimensions"]["a_grounded"] = {
            "verdict": "fault",
            "why": "UNGROUNDED (still no specific cite, attempt "
                   f"{attempt})",
        }
        return json.dumps(rep)
    r = _council.run_council({}, {}, linter_grounded_true, stub)
    if r.disposition != "proceed":
        return False, (f"council halted on persistent spurious a_grounded "
                       f"fault instead of resolving to pass: "
                       f"disposition={r.disposition!r}, "
                       f"halt_reason={r.halt_reason!r}, "
                       f"retries={r.retries_consumed}")
    if r.overall != "pass":
        return False, f"overall != pass after spurious-fault resolution"
    if r.retries_consumed != _council.A_GROUNDED_MAX_RETRIES:
        return False, (f"retries_consumed should equal "
                       f"A_GROUNDED_MAX_RETRIES="
                       f"{_council.A_GROUNDED_MAX_RETRIES}; got "
                       f"{r.retries_consumed}")
    a_cell = r.report["dimensions"]["a_grounded"]
    if a_cell["verdict"] != "pass":
        return False, ("a_grounded cell verdict not rewritten to pass "
                       "after spurious-fault reconciliation")
    if "spurious" not in (a_cell.get("why") or "").lower():
        return False, ("a_grounded.why does not document the spurious-"
                       "fault reconciliation audit trail")
    return True, (f"persistent spurious a_grounded:fault overridden to "
                  f"pass after {_council.A_GROUNDED_MAX_RETRIES} retries; "
                  f"chain proceeds")


# ---- C-A-GROUNDED-NAMED-GAP (v6 NEW) ------------------------------------
# When the deterministic linter has FLAGGED a specific cite as unmatched
# (grounded=false), and the synth emits a_grounded:fault NAMING that same
# cite, the fault is genuine and the chain MUST still terminate. The v6
# fix must not blunt the bright-line on real grounding faults.
@check("C-A-GROUNDED-NAMED-GAP")
def _c_a_grounded_named_gap():
    sys.path.insert(0, str(SEALED / "code"))
    import importlib, council as _council
    importlib.reload(_council)
    DIMS = _council.DIMS

    flagged_cite = "A_market_bars NVDA 2026-05-29 close 1180"
    linter_grounded_false = {
        "grounded": False,
        "citation_map": {
            "A_market_bars AMZN 2026-05-22 close 266.27": True,
            flagged_cite: False,
            "O_own_account 2026-05-29T13:35": True,
        },
        "ungrounded_reason": f"cited refs not in payload: [{flagged_cite!r}]",
    }
    state = {"calls": 0}
    def stub(attempt):
        state["calls"] += 1
        ...
        rep["dimensions"]["a_grounded"] = {
            "verdict": "fault",
            "why": (f"UNGROUNDED: cited '{flagged_cite}' but no NVDA "
                    f"row exists in A_market_bars for this wake."),
        }
        return json.dumps(rep)
    r = _council.run_council({}, {}, linter_grounded_false, stub)
    if r.disposition != "halt":
        return False, (f"council did NOT terminate on a genuine "
                       f"a_grounded fault (synth named a "
                       f"linter-flagged cite): disposition="
                       f"{r.disposition!r}, halt_reason={r.halt_reason!r}")
    if "bright_line_fault:a_grounded" not in (r.halt_reason or ""):
        return False, (f"halt_reason does not indicate a_grounded "
                       f"bright-line: {r.halt_reason!r}")
    if r.retries_consumed != 0:
        return False, (f"genuine a_grounded fault should terminate "
                       f"without consuming retries; got "
                       f"{r.retries_consumed}")
    if state["calls"] != 1:
        return False, (f"synth called {state['calls']} times on a "
                       f"genuine first-draw fault (expected 1)")
    return True, ("genuine a_grounded fault (synth named "
                  "linter-flagged cite) terminates as designed; "
                  "0 retries consumed")


def main() -> int:
    print("=== KUWAI commitment-v6 BINARY CONFORMANCE GATE (23 gates) ===")
    all_pass = True
